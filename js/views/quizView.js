...
export function renderQuizView(state) {
  ...
  const scoreLine = div({ class: 'quiz-score-line' });
  ...
  function renderScore() {
    ...
    replace(scoreLine,
      div({ class: 'quiz-score' }, span({ text: `Score: ${correct} / ${QUIZ.length}` }),
        span({ class: 'subtle', text: `  ·  ${answered} answered` })),
      button({ label: 'Reset quiz', variant: 'ghost', icon: '↺', onClick: () => { answers = {}; renderAll(); } }));
  }
